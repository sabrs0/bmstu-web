...
import (
	...
	metricsMW "github.com/sabrs0/bmstu-web/src/internal/http-server/middleware/metrics"
...
func SetRouter(conn *sql.DB, logger *slog.Logger, metrics *benchmark.Metrics) *mux.Router { //мб сюда передать контроллеры ?
	...
	mainRouter.Use(metricsMW.New(metrics))
	...
	unAuthRouter.HandleFunc("/foundations/{id}/foundrisings",
		foundations.GetFoundrisings(logger, foundationController)).Methods("GET", "OPTIONS")
	unAuthRouter.HandleFunc("/foundrisings/{id}",
	...
	foundationAdminAuthRouter.HandleFunc("/foundations/{id}",
		foundations.Update(logger, foundationController)).Methods("PUT", "OPTIONS")
	foundationAdminAuthRouter.HandleFunc("/foundations/{id}",
		foundations.GetByID(logger, foundationController)).Methods("GET", "OPTIONS")
	foundationAdminAuthRouter.HandleFunc("/foundations/extended/{id}",
		foundations.GetExtByID(logger, foundationController)).Methods("GET", "OPTIONS")
	foundationAdminAuthRouter.HandleFunc("/foundations/extended_login/{login}",
		foundations.GetExtByLogin(logger, foundationController)).Methods("GET", "OPTIONS")
	foundationAdminAuthRouter.HandleFunc("/foundations/{id}",
		foundations.Delete(logger, foundationController)).Methods("DELETE", "OPTIONS")
	foundationAdminAuthRouter.HandleFunc("/foundations/{id}/replenish",
		foundations.Replenish(logger, foundationController)).Methods("POST", "OPTIONS")
	...
	userAdminAuthRouter.HandleFunc("/users/login/{login}", users.GetByLogin(logger, userController)).Methods("GET")
	userAdminAuthRouter.HandleFunc("/users/{id}/replenish",
		users.Replenish(logger, userController)).Methods("POST", "OPTIONS")
