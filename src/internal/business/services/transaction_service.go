...
type ITransactionService interface {
	...
	GetFromId(type_ bool, id_ string, FndS IFoundationService,
		US IUserService) ([]ents.Transaction, error)
	GetFoundrisingPhilantropIds(id_ string, FndgS IFoundrisingService) ([]uint64, error)
	GetToId(type_ bool, id_ string, FndS IFoundationService,
		FndgS IFoundrisingService) ([]ents.Transaction, error)

	ExistsById(id uint64) bool
	GetRepo() repos.ITransactionRepository
}
...
func (TS *TransactionService) GetRepo() repos.ITransactionRepository {
	return TS.TR
}
...
func (TS *TransactionService) GetFromId(type_ bool, id_ string, FndS IFoundationService,
	US IUserService) ([]ents.Transaction, error) {
	sid, err := strconv.Atoi(id_)
	id := uint64(sid)
	var U []ents.Transaction
...
func (TS *TransactionService) GetToId(type_ bool, id_ string, FndS IFoundationService,
	FndgS IFoundrisingService) ([]ents.Transaction, error) {
	sid, err := strconv.Atoi(id_)
	id := uint64(sid)
	var U []ents.Transaction
...
func (TS *TransactionService) GetFoundrisingPhilantropIds(id_ string, FndgS IFoundrisingService) ([]uint64, error) {
	sid, err := strconv.Atoi(id_)
	id := uint64(sid)
	var IDs []uint64
	if err != nil {
